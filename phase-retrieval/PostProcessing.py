...
def centerObject( img, sup ):
    ...
    for n in list( range( len( span ) ) ):
        if 1+span[n].max()-span[n].min() == supC.shape[n]: # object is split across periodic boundary
            imgC = np.roll( imgC, imgC.shape[n]//2, axis=n )
            supC = np.roll( supC, supC.shape[n]//2, axis=n )
    span = np.where( supC > 0.5 )

    y, x, z = np.meshgrid( np.arange( supC.shape[0] ), np.arange( supC.shape[1] ), np.arange( supC.shape[2] ) )
    sm = supC.sum()
    xc = ( x*supC ).sum() // sm
    yc = ( y*supC ).sum() // sm
    zc = ( z*supC ).sum() // sm
